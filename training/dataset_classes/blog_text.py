...
class BlogAuthorshipDataset(Dataset):
    ...
    def __init__(
        ...
        print(
            f"[{split_type}] {len(self.texts)} docs | {self.num_classes} authors | "
...
class BlogAuthorshipDataModule(pl.LightningDataModule):
    ...
    def __init__(
        ...
        self.test_csv_path = (
            test_csv_path  # May be None → use csv_path with disjointness check
        )
    ...
    def _verify_splits(self):
        ...
        assert (
            len(val_probe_set & val_gallery_set)
            == len(self.val_probe_authors_list) // 2
