...
class WhaleDataModule(LightningDataModule):
    def __init__(
        ...
        if fold == -1:
            ...
            print(
                f"new: {(self.val_df.individual_id == cfg.num_classes).sum()} / {len(self.val_df)}"
            )
...
class SphereClassifier(LightningModule):
    def __init__(self, cfg: dict, id_class_nums=None, species_class_nums=None):
        ...
        self.global_pools = torch.nn.ModuleList(
            [
                GeM(p=cfg.global_pool.p, requires_grad=cfg.global_pool.train)
                for _ in cfg.out_indices
            ]
        )
        ...
        if id_class_nums is not None and species_class_nums is not None:
            margins_id = (
                np.power(id_class_nums, cfg.margin_power_id) * cfg.margin_coef_id
                + cfg.margin_cons_id
            )
            margins_species = (
                np.power(species_class_nums, cfg.margin_power_species)
            ...
            self.margin_fn_id = ArcFaceLossAdaptiveMargin(
                margins_id, cfg.num_classes, cfg.s_id
            )
            self.margin_fn_species = ArcFaceLossAdaptiveMargin(
                margins_species, cfg.num_species_classes, cfg.s_species
            )
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss_species = self.loss_fn_species(
        ...
        with torch.no_grad():
            ...
            self.log_dict(
                {
                    "train/acc_species": topk_average_precision(
                        logits_species, species, 1
                    )
                    .mean()
                    .detach()
                },
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log_dict(
            {
                "val/acc_species": topk_average_precision(output_species, species, 1)
                .mean()
                .detach()
            },
            on_step=False,
        ...
        if self.trainer.global_rank == 0:
            ...
            for key in outputs[0].keys():
                if torch.cuda.device_count() > 1:
                    result = torch.cat([x[key] for x in outputs], dim=1).flatten(
                        end_dim=1
                    )
                else:
...
def train(
    ...
    data_module = WhaleDataModule(
        df,
        cfg,
        f"{args.in_base_dir}/train_images",
        cfg.val_bbox,
        fold,
    ...
    if do_inference:
        for test_bbox in cfg.test_bboxes:
            ...
            test_data_module = WhaleDataModule(
                df_test, cfg, f"{args.in_base_dir}/test_images", test_bbox, -1
            )
...
def main():
    ...
    if cfg.pseudo_label is not None:
        ...
        pseudo_dataset = WhaleDataset(
            pseudo_df[pseudo_df.conf > cfg.pseudo_conf_threshold],
            cfg,
