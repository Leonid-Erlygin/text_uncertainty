...
def knn_all_pred(
    ...
    test_dist, test_cosine_idx = neigh.kneighbors(
        test_feat, return_distance=True
...
def knn_both_feat(
    n_class, train_feat1, train_feat2, test_feat1, test_feat2, train_label
):
    # knn with both feats for train
    train_feat12 = np.concatenate([train_feat1, train_feat2], axis=0)
    knn1_both_mat = knn_all_pred(
        n_class, len(train_label), test_feat1, train_feat12, train_label
    )
    knn2_both_mat = knn_all_pred(
        n_class, len(train_label), test_feat2, train_feat12, train_label
    )
...
def binary_search_threshold(n_class: int, mat: torch.Tensor, new_ratio: float) -> float:
    ...
    for _ in range(30):
        mid = (ok + ng) / 2
        out_new = torch.cat(
            [mat, torch.full((mat.shape[0], 1), mid, device=mat.device)], dim=1
        )
...
def make_submission(
    ...
    for train_path, test_path in zip(train_paths, test_paths):
        ...
        logit_mat_csr = scipy.sparse.csr_matrix(
            restore_all_pred(
                cfg.num_classes, test_results["pred_logit"], test_results["pred_idx"]
            )
        )
    ...
    for new_ratio in new_ratios:
        ...
        out_new = torch.cat(
            [out, torch.full((out.shape[0], 1), threshold, device=out.device)], dim=1
        )
        ...
        label_encoder.classes_ = np.load(
            f"{args.in_base_dir}/individual_id.npy", allow_pickle=True
        )
        ...
        def make_str(id_list):
            return " ".join(
                (
                    "new_individual"
                    if x == cfg.num_classes
                    else label_encoder.inverse_transform([x])[0]
                )
                for x in id_list
            )
        ...
        df.to_csv(
            f"submission/{args.out_prefix}-{new_ratio}-{threshold}.csv",
            index=False,
    ...
    df.to_csv(
        f"submission/pseudo_label_{args.out_prefix}.csv",
        index=False,
        columns=["image", "individual_id", "conf", "species"],
    )
