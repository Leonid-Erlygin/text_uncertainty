...
def load_bbox(
    cfg: Config, in_base_dir: str, bbox_name: str, is_train: bool
) -> pd.Series:
    if bbox_name == "detic":
        ...
        tmp_df = pd.read_csv(f"{in_base_dir}/{filename}")
        low_conf = (
            tmp_df.conf.map(lambda s: float(s[1:-1]) if s == s else -1)
            < cfg.bbox_conf_threshold
        )
        ...
        bbox = tmp_df.bbox.map(
            lambda s: list(map(int, s[2:-2].split())) if s == s else None
        )
...
def load_df(
    in_base_dir: str, cfg: Config, filename: str, is_train: bool
) -> pd.DataFrame:
    ...
    if hasattr(df, "individual_id"):
        ...
        label_encoder.classes_ = np.load(
            f"{in_base_dir}/individual_id.npy", allow_pickle=True
        )
    ...
    if hasattr(df, "species"):
        ...
        label_encoder_species.classes_ = np.load(
            f"{in_base_dir}/species.npy", allow_pickle=True
        )
